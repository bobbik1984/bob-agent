...
## 📍 目标 22: Bob-Mobile 手机端 MVP (Milestone 2 - UI 重构与演进)
> 🎯 **目标**: 在已跑通的 Android 底座上，重构并适配适合手机端触屏交互的 UI/UX，探索端侧本地开源模型驱动。

### 📱 界面适配与移动端特化
- [ ] **M2-01: UI 触控与布局重构** — 废弃侧边栏双栏布局，采用手机常见的抽屉 (Drawer) 侧滑导航；放大字号，增加按钮与列表的触控面积 (Touch Target)，彻底从鼠标逻辑转为手指触控逻辑。
- [ ] **M2-02: 沉浸式状态栏与幽灵标题栏移除** — 在移动端彻底隐藏自定义的 PC 版 `- □ X` 标题栏，并配置 Tauri 开启沉浸式全屏 / 透明状态栏 (Translucent Status Bar)，避开时间、电量、刘海屏区域。
- [ ] **M2-03: Onboarding 流程移动端特化** — 移动端默认且固定使用沙盒工作空间，跳过/隐藏“选择本地工作目录”步骤；优化引导页过细的 SVG 图标，使其在小屏幕上对比度更高、更清晰。
- [ ] **M2-04: 专属品牌标志** — 将 Tauri 默认的黄绿色双环图标替换为 Bob 的专属 Logo。

### ✂️ 功能裁剪 (MVP 聚焦)
- [ ] **M2-11: 移除 PC 端社交渠道** — 移动端暂不加载微信、Telegram、Discord 协议收发模块，减轻包体积、网络请求和耗电负担。

### 🧠 架构愿景: 端侧模型驱动框架
- [ ] **M2-21: 探索本地 LLM 推理外壳** — 针对现代手机算力过剩的现状，调研并在 App 中集成类似 `ggml` / `llama.cpp` 的跨端推理框架。
- [ ] **M2-22: 部署开源模型** — 尝试在 Android 端本地跑通轻量级、高智商的开源大模型（如 Gemma 2b / 4b 或 Qwen 1.5B），实现断网可用、数据绝对隐私的终极体验。
