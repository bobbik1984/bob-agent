...
**主题**: 窄屏响应式布局修复与移动端 UI 细节打磨

**完成**:
1. [Fix] **PC 端窄屏 Onboarding 误判修复** — 重写 SetupWizard.vue 逻辑，改用真实 userAgent 检测手机，确保 PC 用户缩小窗口时依然展示微信绑定。
2. [Fix] **设置页连接卡片样式统一** — 在 SettingsConnections.vue 移除多余的 Hover 悬浮交互与边框，强制等高网格，使所有配置卡片高度对齐。
3. [Fix] **知识图谱窄屏容器重叠修复** — 在 KnowledgeGraphView.vue 增加自适应响应式切换，检测到窄屏时改用纵向布局，防止顶部导航栏被异常拉伸霸占屏幕。
4. [Fix] **日程视图顶部标签栏响应式优化** — 在 InboxView.vue 移除窄屏下的左右 padding，并设置等分比例，使得移动端 Tabs 能 100% 满宽贴边展示。
5. [Fix] **手机端顶部状态栏对比度修复** — 在 App.vue 为移动端顶部安全区 safe-area-inset-top 加装纯黑背景遮罩，确保在浅色主题下依然能看清手机系统的白色状态栏文字。
