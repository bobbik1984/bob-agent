<p align="center">
  ...
  <p align="center">
    <img src="https://img.shields.io/badge/version-v0.9.5-blue?style=flat-square" alt="Version" />
...
|:---:|---|---|
| 💼 | **Work Core 持续工作台** | Project-based task, decision, and milestone tracking with continuous execution loops. <br>以项目为核心聚合任务、决策、动态与里程碑，跨会话持续推进业务交付。 |
| 🌅 | **Today Layer 晨报聚焦** | Conversation-first Today summary of focus items, approvals, and daily brief without cloud LLMs. <br>启动即见首要焦点与关注项，断网秒级聚合日程、待办与工作状态。 |
...
| 📲 | **Mobile Companion 移动伴侣** | Native Android client with LAN / Relay E2EE sync, offline access, and remote headless PC task invocation. <br>原生安卓伴侣端，支持局域网/中继端到端加密双向同步与离线查询，并可远程唤醒电脑后台执行。 |
...
- **Credential Storage**: **Local Protected JSON Storage** (`config.json`) in user AppData — 100% private, zero cloud transmission.
...
- `bob-installer.exe` — Custom borderless dark-themed installer (~40MB).
- `bob-agent-portable.zip` — Portable green version (~29MB).
- `bob-mobile-latest.apk` — Native Android companion client APK (~80MB).
...
│   ├── work_core/                  # Continuous Work Core (Projects, Tasks, Decisions)
│   ├── daily_brief/                # Conversation-first Today Layer & Briefing engine
│   ├── goal_runtime/               # Goal Mode execution engine & approval flow
│   ├── sync_engine.rs              # LAN & Relay bidirectional E2EE sync engine
│   ├── capture.rs                  # Multi-entry Quick Capture with journaling
...
│   ├── views/                      # Interactive panels (Chat, Work, Today, Settings)
