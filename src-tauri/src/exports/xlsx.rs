...
pub fn generate_xlsx<P: AsRef<Path>>(path: P, data: &XlsxData) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for sheet_data in &data.sheets {
        ...
        for (row_idx, row) in sheet_data.rows.iter().enumerate() {
            ...
            let current_format = if row_idx % 2 == 1 { &alt_row_format } else { &row_format };
            
            for (col_idx, cell) in row.iter().enumerate() {
                let col_num = col_idx as u16;
                match cell {
                    CellValue::Text(text) => {
                        worksheet.write_string_with_format(row_num, col_num, text, current_format)?;
                    },
                    CellValue::Number(num) => {
                        worksheet.write_number_with_format(row_num, col_num, *num, current_format)?;
                    },
                    CellValue::Empty => {
