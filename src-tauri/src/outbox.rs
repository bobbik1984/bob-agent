...
const KNOWN_PROVIDERS: &[&str] = &[
    "deepseek", "openai", "qwen", "doubao", "zhipu", "kimi", "minimax",
    "tavily", "tinyfish",
];
...
fn reconcile() -> Result<usize, String> {
    ...
    let content = fs::read_to_string(&outbox_path)
        .map_err(|e| format!("读取 Outbox 失败: {}", e))?;
    
    let outbox: Value = serde_json::from_str(&content)
        .map_err(|e| format!("Outbox JSON 解析失败: {}", e))?;

    let operations = outbox.get("operations")
    ...
    for op in operations {
        match validate_operation(op) {
            ...
            Err(reason) => {
                let op_desc = describe_operation(op);
                audit_entries.push(format!("[{}] ❌ REJECTED: {} — {}", timestamp, op_desc, reason));
            }
        }
    }
    ...
    audit_entries.push(format!(
        ...
        timestamp, applied, operations.len() - applied
    ));
...
fn apply_operation(config: &mut Value, op: &Value) {
    ...
    match op_type {
        ...
        "add_tracked_folder" => {
            ...
            if let Some(cfg_obj) = config.as_object_mut() {
                let mut folders: Vec<String> = cfg_obj.get("trackedFolders")
