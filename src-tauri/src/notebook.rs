    ...
    match fs::remove_file(&full_path) {
        Ok(_) => {
            ...
            if let Ok(conn) = db.0.lock() {
                ...
                // Clean wiki_fts (for wiki/sources/* notes)
                if path.starts_with("wiki/sources/") {
                    let file_name = full_path.file_name()
                        .map(|f| f.to_string_lossy().to_string())
                        .unwrap_or_default();
                    let _ = conn.execute(
                        "DELETE FROM wiki_fts WHERE file_name = ?1",
                        rusqlite::params![&file_name],
                    );
                }
                // Clean kg_nodes where source matches this path
                // First collect node IDs to delete, then delete edges and nodes
                let node_ids: Vec<String> = {
                    let mut stmt = conn.prepare(
                        "SELECT id FROM kg_nodes WHERE source = ?1 OR source = ?2"
                    ).unwrap();
                    let file_name = full_path.file_name()
                        .map(|f| f.to_string_lossy().to_string())
                        .unwrap_or_default();
                    stmt.query_map(rusqlite::params![&path, &file_name], |row| {
                        row.get::<_, String>(0)
                ...
                if !node_ids.is_empty() {
                    println!("[notebook] Cascade deleted {} KG nodes for '{}'", node_ids.len(), path);
                }
            }
            Ok(json!({ "ok": true }))
        }
