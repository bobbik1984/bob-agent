...
#[tauri::command]
pub fn notebook_save_note(path: String, content: String, db: State<DbState>) -> Result<Value, String> {
    let full_path = resolve_note_path(&path);
    ...
    match fs::write(&full_path, &final_content) {
        Ok(_) => {
            if let Ok(conn) = db.0.lock() {
                ...
                let wikilink_re = regex::Regex::new(r"\[\[([^\[\]]+)\]\]").unwrap_or_else(|_| regex::Regex::new("$^").unwrap());
...
#[tauri::command]
pub fn notebook_create_note(title: String, tags: Vec<String>, category: Option<String>) -> Result<Value, String> {
        ...
        Ok(_) => {
            ...
            if let Ok(conn) = db.0.lock() {
                ...
                for src in &original_sources {
                    let file_node_id = format!("file_{}", src.to_lowercase().replace(' ', "_").replace('.', "_"));
                    doc_node_ids.push(file_node_id);
                }

                for doc_id in &doc_node_ids {
                    let _ = conn.execute("DELETE FROM kg_edges WHERE source_id = ?1 OR target_id = ?1", rusqlite::params![doc_id]);
                    let _ = conn.execute("DELETE FROM kg_nodes WHERE id = ?1", rusqlite::params![doc_id]);
                }
                ...
                for src in &original_sources {
                    if let Ok(mut stmt) = conn.prepare("SELECT id FROM kg_nodes WHERE source = ?1") {
                        if let Ok(rows) = stmt.query_map(rusqlite::params![src], |row| row.get::<_, String>(0)) {
                ...
                if !nodes_to_delete.is_empty() {
                    println!("[notebook] Cascade deleted {} KG nodes for '{}'", nodes_to_delete.len(), path);
                }
            }
            Ok(json!({ "ok": true }))
        }
...
pub fn notebook_list_all_tags() -> Result<Value, String> {
    ...
    let mut tags_list: Vec<Value> = tag_counts.into_iter()
...
pub fn notebook_update_tags(path: String, tags: Vec<String>) -> Result<Value, String> {
    ...
    let new_fm = if raw_content.starts_with("---") {
        if let Some(end_idx) = raw_content[3..].find("---") {
            ...
            fm.tags = tags;
            fm.updated = chrono::Local::now().format("%Y-%m-%dT%H:%M:%S%z").to_string();
        ...
        let fm = NoteFrontmatter {
            title: full_path.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_default(),
            tags,
            created: chrono::Local::now().format("%Y-%m-%dT%H:%M:%S%z").to_string(),
            updated: chrono::Local::now().format("%Y-%m-%dT%H:%M:%S%z").to_string(),
        };
        let yaml = serde_yaml::to_string(&fm).unwrap_or_default();
        format!("---\n{}---\n\n{}", yaml, raw_content)
    };
...
pub fn notebook_get_backlinks(path: String) -> Result<Value, String> {
    ...
    let title = if let Ok(raw) = fs::read_to_string(&full_path) {
        if raw.starts_with("---") {
            if let Some(end_idx) = raw[3..].find("---") {
                ...
                if let Ok(fm) = serde_yaml::from_str::<NoteFrontmatter>(fm_body) {
                    if !fm.title.is_empty() { fm.title } else {
                        full_path.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_default()
                    }
                } else {
                    full_path.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_default()
    ...
    fn scan_dir(dir: &std::path::Path, notes_dir: &std::path::Path, pattern: &str, backlinks: &mut Vec<Value>) {
        if let Ok(entries) = fs::read_dir(dir) {
            for entry in entries.flatten() {
                let entry_path = entry.path();
                if entry_path.is_dir() && entry_path.file_name().map(|n| n != "assets").unwrap_or(true) {
                    scan_dir(&entry_path, notes_dir, pattern, backlinks);
                } else if entry_path.extension().map(|e| e == "md").unwrap_or(false) {
                    if let Ok(content) = fs::read_to_string(&entry_path) {
                        if content.contains(pattern) {
                            let rel_path = entry_path.strip_prefix(notes_dir)
...
#[tauri::command]
pub fn notebook_merge_tags(canonical: String, aliases: Vec<String>, db: State<DbState>) -> Result<Value, String> {
    let notes_dir = get_notes_dir();
    ...
    // Write merge history
    let data_dir = get_notes_dir().parent().unwrap_or(&get_notes_dir()).to_path_buf();
...
pub fn notebook_reject_tag_merge(tag_a: String, tag_b: String) -> Result<Value, String> {
    ...
    let mut exclusions: Vec<(String, String)> = if let Ok(raw) = std::fs::read_to_string(&exclusions_path) {
        serde_json::from_str(&raw).unwrap_or_default()
    } else { vec![] };
    
    // Normalize: always store sorted pair
    let pair = if tag_a < tag_b { (tag_a, tag_b) } else { (tag_b, tag_a) };
    if !exclusions.contains(&pair) {
        exclusions.push(pair);
    }
    let _ = std::fs::write(&exclusions_path, serde_json::to_string_pretty(&exclusions).unwrap_or_default());
