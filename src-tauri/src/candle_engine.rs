...
#[tauri::command]
pub async fn candle_start_offline_engine(model_path: String, state: State<'_, CandleState>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn candle_stop_offline_engine(state: State<'_, CandleState>) -> Result<serde_json::Value, String> {
    *state.is_running.lock().unwrap() = false;
...
#[tauri::command]
pub async fn candle_get_offline_engine_status(state: State<'_, CandleState>) -> Result<serde_json::Value, String> {
    ...
    if !running {
        ...
        if let Some(path) = config.get("offlineModelPath").and_then(|v| v.as_str()) {
            if !path.is_empty() {
                ...
                let _ = candle_start_offline_engine(path.to_string(), state.clone()).await;
                let _ = tx.send(());
            }
        }
    }
