...
pub async fn refresh_tenant_token() -> Result<String, String> {
    let creds = super::connector::load_credentials("lark")
        .ok_or("飞书未配置凭证")?;
    ...
    let resp = client
        .post(format!("{}/auth/v3/tenant_access_token/internal", LARK_API_BASE))
        .json(&json!({
            "app_id": app_id,
    ...
    let body: Value = resp.json().await.map_err(|e| format!("飞书响应解析失败: {}", e))?;
    ...
    log::info!("[Lark] tenant_access_token refreshed, expires in {}s", expire);
    Ok(token)
}
...
pub async fn execute_tool(name: &str, args: &Value) -> Value {
    match name {
        "lark_list_calendar_events" => {
            let start = args.get("start_time").and_then(|v| v.as_str()).unwrap_or("");
        ...
        "lark_send_message" => {
            tool_send_message(args).await
        }
        "lark_create_bitable_record" => {
            tool_create_bitable_record(args).await
        }
...
async fn tool_send_message(args: &Value) -> Value {
    let receive_id = args.get("receive_id").and_then(|v| v.as_str()).unwrap_or("");
    let receive_id_type = args.get("receive_id_type").and_then(|v| v.as_str()).unwrap_or("chat_id");
    let msg_type = args.get("msg_type").and_then(|v| v.as_str()).unwrap_or("text");
