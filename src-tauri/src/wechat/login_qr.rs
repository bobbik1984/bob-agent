...
#[tauri::command]
pub async fn wechat_check_login_status(
    qrcode: String,
    state: State<'_, Arc<WechatState>>,
) -> Result<Value, String> {
    let url = format!(
        ...
        if status == "confirmed" || status == "binded_redirect" {
            ...
            if !account_id.is_empty() && !token.is_empty() {
                ...
                *state.account_id.write().unwrap() = Some(account_id.clone());
                ...
                data.as_object_mut()
                    .unwrap()
                    .insert("accountId".to_string(), json!(account_id));
            }
        }
    }
