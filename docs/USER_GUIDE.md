...
- “工作”统一承载项目概览、目标、任务、决策和动态，不需要在不同页面重复建立项目。
- 宽屏 PC 在左侧栏显示这五个子项；缩窄窗口或使用 Android 时，它们移动到页面顶部。两种布局内容相同，改变窗口宽度不会丢失当前选中项。
- 当前子项使用主题色实色块表示。项目名称本身可展开或收起，右侧数字显示该项目在当前子项中的内容数量。
- 多个项目可以同时展开，便于全盘查看；收起只改变显示，不会暂停或删除项目。
- 页面不再显示口号式题头或重复的项目选择器。新建项目入口位于项目列表上方。
