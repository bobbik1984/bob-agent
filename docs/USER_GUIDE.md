...
Goal Mode（目标模式）目前是 Bob 的高预算执行原型。它采用 **Maker-Checker（执行-评审）架构**：主模型执行，Clerk 独立复核，并在未通过时有限重试。它可以减少用户手动催促，但不保证一定成功，也不能在应用重启后自动续跑。

### 如何激活

在发送消息时，切换到 **Goal Mode** 开关即可启用。

### 运行机制

1. **Maker（执行端）**：使用 Main 模型进行任务执行。工具调用预算从默认的 5 轮提升至 **50 轮上限**，允许 AI 进行极其复杂的链路操作（读取文件 → 搜索网页 → 撰写内容 → 自我纠错）。
2. **Checker（评审端）**：调用 Clerk 模型作为严格评判器。它会读取你的原始目标和 Maker 的最终输出，以 `FAIL` 为默认立场，逐条核验成果。
3. **有限重试**：如果 Checker 判定不通过，它会生成缺失清单并交给 Maker 修正；外层最多尝试 3 轮。达到预算仍未通过时应停止并明确说明未完成原因。
...
> 当前限制：Auto 模式不会自动升级为 Goal；持久 Goal、任务 DAG、证据状态和重启恢复仍在开发路线中。
