...
Goal Mode（目标模式）是 Bob 的高阶执行模式。它采用 **Maker-Checker（执行-评审）闭环架构**，让 AI 像一个严谨的工程师一样反复打磨任务成果，直到完全达标。
...
3. **自动重试**：如果 Checker 判定不通过，它会生成详细的缺失清单（Feedback），系统将此 Feedback 封装为新任务重新交给 Maker 修正。整个循环**最多重试 3 次**外环，直到 Checker 给出 `PASS` 判定。
