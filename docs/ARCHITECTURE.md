...
`assistant_context.rs` 已实现 Phase 5.5-B 的首个只读切片：把最后一条用户输入编译为 `PurposeFrame`，从现有 Work Core 读取活跃 Project 与聚合，按显式 ID、完整标题、Work Object 标题和唯一当前项目等确定性信号生成候选，再形成带 source、revision、updated_at 和字符预算的 `AssistantContext`。Direct、Deep、Advanced 分别限制为 6/12/20 条事实和 3,000/8,000/16,000 字符。

```text
最后一条用户输入 + RouteMode
        │
        ├─ PurposeFrame（目的、显式约束、候选引用、能力需求提示）
        ├─ Work Core 只读快照（不读取聊天历史，不新增表）
        └─ 确定性候选与预算裁剪
                    │
                    └─ AssistantContext + reason codes
                               └─ 默认 shadow：只记指标，不注入模型
```
