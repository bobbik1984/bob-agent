...
PC 与 Android 的“工作”页共用同一套概览、目标、任务、决策和动态视图，中间按最近更新顺序展示全部项目。宽屏 PC 复用日程和设置页面的左侧二级导航；窗口变窄或运行在 Android 时，同一组功能切换到贴合内容区边界的顶部菜单，当前选中项使用主题色实色块。项目标题可独立展开或收起，收起状态直接显示当前视图的内容数量；多个项目可以同时展开，以支持跨项目全盘查看。不再保留旧口号题头、横向项目选择器或另一套内容结构。
...
- Work 的宽屏侧栏与窄屏顶部菜单共享同一视图状态，调整窗口宽度不会重置当前页面，也不会同时出现两套导航。
- Quick Note 的输入条始终以可用屏幕居中；底部 3–5 个入口与输入条左右边界一致、等宽排列，Today 数量保持在胶囊中心线上。
- Windows 安装器以隐藏窗口方式执行必要的进程检查，保持静默安装体验且不引入外部运行时。
...
- **Source-Hub 架构聚合**：将 Web、微信、Discord、本地文件等多渠道异构数据清洗后作为大 Hub 节点入库，支持精确溯源与级联清除
...
### 4. 结构化数据检索引擎 (Native Table Tools)
- **极速解析**：底层集成 Rust `calamine` 引擎，实现对庞大 Excel/CSV 报表的极速解析。
- **智能抽提 (Smart Extraction)**：
  - `table_schema_viewer`: 秒级查阅表头结构。
  - `table_global_search`: 跨列关键词匹配。
  - `table_column_filter`: 针对特定列的漏斗筛选。
- **防截断保护**：所有工具单次最大返回 200 行记录，从根源阻断 Token 爆炸。
