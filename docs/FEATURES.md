...
PC 与 Android 的“工作”页共用同一套功能视图：顶部可在概览、目标、任务、决策和动态之间切换，中间按最近更新顺序展示全部项目。项目标题可独立展开或收起，收起状态直接显示当前视图的内容数量；多个项目可以同时展开，以支持跨项目全盘查看。桌面端只调整最大宽度和留白，不再保留旧口号题头、横向项目选择器或另一套内容结构。
