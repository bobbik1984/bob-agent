...
- [ ] 正式路线图包含 Phase 5.5，且不把未实现能力标记为完成；
- [ ] 基线数据可重复采集，缺失项明确标为未测；
- [ ] PurposeFrame 和 AssistantContext 契约有稳定序列化测试；
- [ ] Context Resolver 不新增数据库表或依赖；
- [ ] 唯一高置信度项目可从一句目的恢复；
- [ ] 两个合理候选时不自动绑定；
- [ ] 上下文包有来源、revision、时效和严格预算；
- [ ] Context Resolver 不改变 Complexity Router 或 R0–R3 权限；
- [ ] 关闭开关或解析失败时可无损回退；
- [ ] 专用 Rust 测试、全量 Rust 测试、前端测试、生产构建和检查通过，或准确记录既有阻塞；
- [ ] 无新增用户侧运行时、常驻服务或安装步骤；
- [ ] 文档只描述真实完成状态。
