...
> 核心主旨：**暗色优先 · 亮暗双栖 · 高级质感 · 绝对对齐 · 语义化色彩 · 文案克制**
...
## 15. 文案克制铁律 — "UI 本身就是说明书"

用户不是第一天用电脑。如果 UI 控件（标题、输入框、按钮、卡片）本身已经说明了功能，文字就是噪音。

### 核心规则：
- **标题自明 → 删除描述**：如果 `section-desc` 是 `section-title` 的同义复述（如标题"记忆管理"+ 描述"查看和管理 Bob 的知识库条目与对话记忆"），**直接删除描述**。
- **控件自明 → 删除描述**：用户已经看到了路径选择框 + 浏览按钮的组合，不需要被告知"配置后 Bob 可以读取该目录"。
- **Onboarding 已教过的不重复**：引导流程中介绍过的功能（如模型自动发现），在设置页不需要再解释一遍。
- **标杆页面**：`外观与语言` 页面——零描述、零废话。所有设置页应以此为标杆。

### 判断流程：
```
描述是否是标题的同义词？ → 删除
描述是否解释了 UI 本身已经呈现的事？ → 删除
描述是否重复了 Onboarding 教过的内容？ → 删除
描述是否包含用户必须知道的操作限制/副作用？ → 保留（考虑 tooltip）
```

### 禁止行为：
```vue
<!-- ❌ 标题叫"模型中心"，下面还写"自动从 .env 发现可用模型" -->
<h3>模型中心</h3>
<p class="section-desc">自动从 .env 密钥库和模型注册表发现可用模型...</p>

<!-- ✅ 标题 + 池计数徽章已经说明一切 -->
<h3>模型中心 <span class="badge">127 个可用模型</span></h3>
```
...
标签（label）中的每个字都有成本。如果上下文已经暗示了含义，去掉冗余词。

### 核心规则：
- **删"选择"**：用户正站在选择界面上，标签写"选择服务商"中的"选择"是冗余的。→ 直接写**"服务商"**。
- **删"管理/配置"**：设置页本身就是管理和配置，标签中再写"管理"是套娃。→ "API 密钥管理" → **"API 密钥"**。
- **删"设置"**：用户已经在设置页里了。→ "外观设置" → **"外观"**。
- **禁 emoji 前缀**：标题和 label 中严禁使用 emoji（如 🔧、💡）。图标用 Lucide 组件，不用 Unicode emoji。
...
图标必须与其所代表的事物在语义上匹配。当已知服务有品牌 Logo 时，必须使用品牌 Logo 而非通用图标。

### 核心规则：
- **已知服务用品牌 Logo**：Google Calendar MCP 必须显示 Google 彩色 G 图标，而不是通用的 `>_` 终端图标。Outlook 同理。
- **未知/自定义服务用通用图标**：只有用户手动添加的自定义 MCP Server 才显示 Terminal 图标。
- **Logo 存放在 `public/logos/`**：所有第三方品牌图标统一放在此目录，使用绝对路径引用。

### 判断流程：
```
服务名包含已知品牌名（google/outlook/telegram等）？
  → 使用 public/logos/ 中对应的品牌图标
否则？
  → 使用 Lucide 通用图标（Terminal/Unplug/Globe 等）
```
...
同一个卡片/面板内，所有表单行的标签宽度、状态指示器位置、输入框起始位置必须严格垂直对齐。

### 核心规则：
- **同一面板内 label 宽度统一**：如果"模型供应商密钥"区的 label 是 `width: 160px`，那么下方"插件/外部服务密钥"区的 label 也必须是 `160px`。不同区域使用不同宽度会导致状态圆点和输入框无法对齐。
- **状态圆点对齐**：每行的绿色/灰色状态圆点必须在相同的水平位置。
- **按钮对齐**：每行末尾的"保存"按钮和删除按钮必须在相同的水平位置。

### 检查方法：
截图后画一条垂直线，检查所有表单行的三个锚点是否共线：
1. Label 右边缘
2. 状态圆点中心
3. 保存按钮左边缘
...
## 19. 实现细节隐藏铁律 — "用户不需要知道引擎盖下面的东西"

技术实现细节不应暴露给用户。用户关心的是"做什么"，不是"怎么做的"。

### 核心规则：
- **隐藏命令行参数**：已配置的 MCP 预设（如 Google Calendar）不应显示 `npx -y @cocal/go...` 这样的底层命令。只有自定义 MCP 才展示命令行（方便专业用户排查）。
- **隐藏加密方式**："所有密钥都已通过操作系统级加密 (safeStorage) 存储" 这种话用户不需要知道。标题写"安全存储"已经足够暗示了。
- **隐藏内部术语**："边车 (sidecar)"、"互斥"等后端/架构术语不应出现在面向用户的 UI 中。如果必须表达互斥逻辑，用 UI 交互实现（如激活一个自动灰掉其他选项）。
- **例外**：安装指南、错误详情等"仅在需要时展开"的信息可以保留在折叠区域内。
