...
```mermaid
graph TB
    Start[用户输入商业问题] --> Step0[STEP 0: 环境检测]
    Step0 --> Step1[STEP 1: 定义问题边界]
    Step1 --> Step2[STEP 2: Issue Tree MECE拆解]
    Step2 --> Step3[STEP 3: 形成Hypotheses]
    Step3 --> Step4[STEP 4: 确定论证方式]
    Step4 --> Step5[STEP 5: 设计Dummy Pages]
    Step5 --> Step6[STEP 6-7: 逐页数据收集+生成]
    Step6 --> Step8[STEP 8: 可选Word]
    Step8 --> Step9[STEP 9: 迭代优化]
    Step9 --> Final[终极版交付]
```
...
|:-----|:-----|:-----|:-----|
| 0 | 环境检测 | 1分钟 | 无 |
| 1 | 问题定义 (Is & Isn't) | 5分钟 | 无 |
| 2 | Issue Tree (MECE拆解) | 8分钟 | methodology.md |
| 3 | Hypotheses (假设+快速搜索) | 12分钟 | methodology.md |
| 4 | 论证方式设计 | 10分钟 | layouts.md + design-specs.md |
| 5 | Dummy Pages 设计 | 15分钟 | page-dependencies.md |
| 6-7 | 逐页数据收集+生成 | 30分钟 | excel-data-spec.md |
| 8 | 可选Word | 10分钟 | delivery-summary.md |
| 9 | 迭代优化 | 10-20分钟 | troubleshooting.md |
| **总计** | | **90-110分钟** | |

### 关键成功因素

1. **问题定义要准确** — Is & Isn't 必须清晰
2. **Issue Tree 要 MECE** — 每层不重不漏，先做 2-3 层
3. **假设要可验证** — 知道什么数据能证明/证伪
4. **Dummy Page 要详细** — 精确到图表类型、X/Y 轴、数据系列
5. **数据要多源验证** — 关键数据至少 2-3 个来源
6. **PPT 要迭代优化** — 初版不求完美，2-3 轮迭代

---

## ⭐ 高级方法论执行指令

以下指令在 STEP 3（Hypothesis 形成）和 STEP 5（Dummy Pages 设计）完成后**必须执行**。
深度理论参考请 `file_read(theory.md)`。

### 1. Pyramid Principle 检查 (STEP 5 完成后)

Dummy.md 的第 2 页（执行摘要）必须遵循金字塔结构：

```
Peak（顶层结论）
  ├── Core Argument 1 (MECE)
  ├── Core Argument 2 (MECE)  ← 严格遵守 "Rule of 3"
  └── Core Argument 3 (MECE)
       └── Evidence（数据/图表证明每个 Argument）
```

**检查项**:
- [ ] Peak 是否是一个明确的、可行动的建议/结论？
- [ ] 是否有恰好 3 个 MECE 的核心论据支撑？
- [ ] 30 秒规则：只看 Peak 能否理解全部建议？

### 2. SCQA 叙事框架 (Executive Summary 和章节开篇)

```markdown
**Situation**: [不可争议的事实背景 — 让听众点头同意]
**Complication**: [打破现状的变化/威胁 — 制造紧迫感]
**Question**: [由 Complication 引出的核心问题]（可省略）
**Answer**: [= Pyramid 的 Peak = 最终建议]
```

**示例**:
```
S: 全球网络安全市场正以 20% CAGR 高速增长
C: 但我司传统本地部署架构无法捕获增长，过去两季度市占率流失 5%
Q: 如何重新获得增长动力？
A: 12 个月内通过收购云原生创业公司，转型 SaaS 模式
```

### 3. Horizontal + Vertical Logic 自检 (STEP 5 完成后)

**Horizontal Logic（横向逻辑）**:
- 将 Dummy.md 中所有页面的 Action Title 按顺序排列
- 只读标题，不看数据 — 是否构成一个连贯的故事？
- 如果不连贯 → 调整页面顺序或重写标题

**Vertical Logic（纵向逻辑）**:
- 对每一页：该页的数据/图表是否**充分证明**了 Action Title 的主张？
- 如果证据不足 → 增加数据需求或修改标题

### 4. "So What?" Test (每个 Action Title 必检)

```
观察 → "So What?" → 洞察 → 行动

❌ "Q3 收入下降了 12%"（纯观察）
✅ "Q3 收入下降 12%，原因是欧洲供应链瓶颈，需立即多元化供应商"（洞察+行动）
```

**规则**: 如果 Action Title 只描述数据而没有方向性洞察，**必须重写**。

### 5. Synthesis vs Summary 区分

- ❌ **Summary（总结）**: "数据显示原材料成本上升 10%，收入下降 5%"
- ✅ **Synthesis（综合）**: "成本上升 + 收入下降 = 当前高量低利模式不可持续 → 需立即裁员 15% 并转向高端产品线"

**规则**: 报告中所有结论必须是 Synthesis，不能是 Summary。
